...
| `RESEND_API_KEY` | `programar-publicacion`, `mfa-enviar-codigo`, `suscribirse`, `confirmar-suscripcion`, `cancelar-suscripcion` | ya configurado (dominio verificado, SMTP de Auth también usa Resend). Tolerante en `programar-publicacion` y `suscribirse`/`confirmar`/`cancelar` (si falta, se loguea y sigue sin tumbar la función). En `mfa-enviar-codigo` es obligatorio |
| `RESEND_AUDIENCE_ID` / `NEWSLETTER_FROM` | `programar-publicacion`, `suscribirse`, `confirmar-suscripcion`, `cancelar-suscripcion` | audiencia de Resend "marketing" y remitente. `confirmar`/`cancelar-suscripcion` sincronizan ahí el estado del contacto (issue #64) — sin `RESEND_AUDIENCE_ID` la confirmación se guarda en `suscriptores_newsletter` pero el contacto nunca entra a la audiencia que recibe el broadcast |
