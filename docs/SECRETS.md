...
| `GH_DISPATCH_TOKEN` | `programar-publicacion`, `notificar-publicacion` | PAT de GitHub con permiso de `repository_dispatch` sobre el repo. NO puede llamarse `GITHUB_*` (prefijo reservado) |
| `GH_DISPATCH_REPO` | `programar-publicacion`, `notificar-publicacion` | opcional, `owner/repo`. Default `mecottl/privas-magazine` |
| `RESEND_API_KEY` | `programar-publicacion`, `notificar-publicacion`, `mfa-enviar-codigo`, `suscribirse`, `confirmar-suscripcion`, `cancelar-suscripcion` | ya configurado (dominio verificado, SMTP de Auth también usa Resend). Tolerante en todas menos `mfa-enviar-codigo` (si falta, se loguea y sigue sin tumbar la función) |
